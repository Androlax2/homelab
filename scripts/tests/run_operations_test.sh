...
test_runs_pending_operations_in_name_order() {
    ...
    run_operations --after || fail_with "expected success"
...
test_failure_stops_the_run_and_is_retried() {
    ...
    touch "$FLAKY_MARKER"
    run_operations --after || fail_with "expected the retry to succeed"
    assert_content 'first\nflaky\nlast' "$OPERATIONS_LOG"
}
...
test_operations_get_no_input() {
    ...
    run_operations --after <<<"yes" || fail_with "expected success"
...
test_before_runs_only_before_operations() {
    add_operation 2026_01_01_000000_prepare.before.sh 'echo prepare >> "$OPERATIONS_LOG"'
    add_operation 2026_01_02_000000_fix.sh 'echo fix >> "$OPERATIONS_LOG"'
    run_operations --before || fail_with "expected success"
    assert_content 'prepare' "$OPERATIONS_LOG"
    assert_content '2026_01_01_000000_prepare.before.sh' "$repo_dir/.operations-done"
}

test_after_leaves_before_operations_alone() {
    add_operation 2026_01_01_000000_prepare.before.sh 'echo prepare >> "$OPERATIONS_LOG"'
    add_operation 2026_01_02_000000_fix.sh 'echo fix >> "$OPERATIONS_LOG"'
    run_operations --after || fail_with "expected success"
    assert_content 'fix' "$OPERATIONS_LOG"
}

test_needs_to_be_told_which_moment() {
    add_operation 2026_01_01_000000_once.sh 'echo ran >> "$OPERATIONS_LOG"'
    if run_operations; then
        fail_with "expected a usage error"
    fi
    grep -q 'Usage:' "$sandbox/output" || fail_with "expected the usage"
    assert_content '' "$OPERATIONS_LOG"
}

test_mark_all_done_records_both_moments_without_running() {
    add_operation 2026_01_01_000000_old.before.sh 'echo ran >> "$OPERATIONS_LOG"'
    add_operation 2026_01_02_000000_old.sh 'echo ran >> "$OPERATIONS_LOG"'
    run_operations --mark-all-done || fail_with "expected success"
    run_operations --before || fail_with "expected success"
    run_operations --after || fail_with "expected success"
    assert_content '' "$OPERATIONS_LOG"
    assert_content '2026_01_01_000000_old.before.sh\n2026_01_02_000000_old.sh' "$repo_dir/.operations-done"
}

test_list_shows_the_moment_and_what_ran() {
    add_operation 2026_01_01_000000_old.sh 'true'
    run_operations --after || fail_with "expected success"
    add_operation 2026_01_02_000000_new.before.sh 'true'
    run_operations --list || fail_with "expected success"
    assert_content 'done     after   2026_01_01_000000_old.sh\npending  before  2026_01_02_000000_new.before.sh' "$sandbox/output"
}

test_refuses_to_run_while_a_deploy_holds_the_lock() {
    add_operation 2026_01_01_000000_once.sh 'echo ran >> "$OPERATIONS_LOG"'
    if flock "$repo_dir/.deploy.lock" bash "$repo_dir/scripts/run_operations.sh" --after > "$sandbox/output" 2>&1; then
...
run_test "--before runs only the operations named .before.sh" in_sandbox test_before_runs_only_before_operations
run_test "--after leaves the .before.sh operations alone" in_sandbox test_after_leaves_before_operations_alone
run_test "it refuses to run without being told which moment" in_sandbox test_needs_to_be_told_which_moment
run_test "--mark-all-done records the pending operations of both moments without running them" in_sandbox test_mark_all_done_records_both_moments_without_running
run_test "--list shows each operation's moment and whether it ran" in_sandbox test_list_shows_the_moment_and_what_ran
